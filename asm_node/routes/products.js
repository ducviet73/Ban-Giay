...
router.post('/api/products',[authen], async function(req, res, next) {
    try {
        let { name, description, old_price, new_price, image, product_type } = req.body;
        const product = await productController.create(name, description, old_price, new_price, image, product_type);
...
router.put('/api/products/:id', async function(req, res, next) {
    try {
        let { id } = req.params;
        let { name, description, old_price, new_price, image, product_type } = req.body;
        const product = await productController.update(id, name, description, old_price, new_price, image, product_type);
        res.status(200).json({ product });
    } catch (error) {
        res.status(500).json({ error: error.message });
    }
